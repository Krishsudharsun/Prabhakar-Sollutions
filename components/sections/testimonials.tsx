        ...
        <StaggerGroup className="mt-16 grid gap-6 sm:grid-cols-2">
          {testimonials.map((t) => (
            <StaggerItem key={t.name}>
              <figure className="h-full rounded-2xl border border-navy-900/[0.06] bg-white p-7 dark:border-white/[0.08] dark:bg-white/[0.045]">
                <Quote className="h-6 w-6 text-royal-600/30" />
                <blockquote className="mt-4 text-base leading-relaxed text-ink dark:text-white/90">
                  "{t.quote}"
                </blockquote>
                <figcaption className="mt-6 flex items-center justify-between border-t border-navy-900/[0.06] pt-4 dark:border-white/[0.08]">
                  <div>
