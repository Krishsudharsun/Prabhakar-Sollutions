...
export async function sendLeadNotification({ subject, heading, fields }: SendLeadEmailInput) {
  ...
  const rows = fields
    .map(
      (f) =>
        `<tr><td style="padding:6px 12px;color:#64748B;font-family:sans-serif;font-size:14px;">${f.label}</td><td style="padding:6px 12px;font-family:sans-serif;font-size:14px;color:#0F172A;">${f.value}</td></tr>`
    )
    .join("");

  try {
    await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL ?? "Prabhakar Solutions <hello@prabhakarsolutions.example>",
      to,
      subject,
      html: `
